...
\abstract{Yoruba has a set of bisyllabic verbs that obligatorily split around a direct object, as in \textit{Ad\'{e} \textbf{ba} il\'{e} n\`{a}\'{a} \textbf{j\'{e}}}, meaning ‘Ad\'{e} \textbf{destroyed} the house’, where both \textit{ba} and \textit{j\'{e}} make up the verb for \textit{destroy}.  These are called ``splitting verbs'' and have previously been analyzed as requiring that the first verbal element be merged directly on \textit{v}. We introduce new data using an aspectual marker, \textit{t\'{u}n}, meaning \textit{again}, which changes the typical word order such that both verbal elements appear string adjacent following the object, as in \textit{Ad\'{e} t\'{u}n il\'{e} n\`{a}\'{a} \textbf{baj\'{e}}}, meaning ‘Ad\'{e} \textbf{destroyed} the house again’. This data supports a movement-based analysis of splitting verbs where both verbal elements are initially merged low in the structure, but the first verbal element is moved through Asp to \textit{v}.}
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{Structure meaning ‘again’ \REF{ex:parrish:again}}
