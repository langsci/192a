...
In \isi{regressive assimilation}, debuccalization, the first step, satisfies the Coda Condition (\textsc{CodaCond}), which is violated by place features that are not associated with an onset. This constraint motivates deleting the place features from the coda consonant, which violates \textsc{Max(Place)}. \tableauref{diola:one} \todo{would you like {\hand} instead of {$\to$} in Tableaux?}
