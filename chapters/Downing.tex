...
\ChapterDOI{10.5281/zenodo.3367128}
...
\z
\z
The similarity in the \isi{tone} of the \ili{Digo} phrases in 13a, b) with the \ili{Tumbuka} verb+ob\-ject phrases illustrated in \REF{ex:downing:4} is striking.
...
\ex\label{ex:downing:16b}\textsc{Dep-Pos}: Every output TBU must have the same High \isi{tone} in the input.
\z
\z
...
\hline
\small
\underline{á}{}-na-henza mu-gaanga & \textsc{Faith-H} & \textsc{AR(H,PP)} & \textsc{F-Pos}\\
 \hline\hline
\underline{á}{}-na-henza mu-gaanga &  & $\ast$! & \\
\hline
{\hanr}\underline{a}{}-na-henza mu-gaángâ &  &  & $\ast$\\
\hline
\end{tabular}
}
\z
...
%  \setlength\doublerulesep{0.5pt}
  \hline
  \normalsize
  ku-henza mu-gaanga & \textsc{Faith-H} & \textsc{AR(H,PP)}) & \textsc{F-Pos}\\
 \hline\hline
  {\hanr} ku-henza mu-gaanga & & &\\
  \hline
...
\OTbox{
  ...
  \hline
  \normalsize
  á{}-na-henza  mu-gaanga & \textsc{Max-H} & \textsc{AR}\par\textsc{(H,PP}) & \textsc{AR}\par\textsc{(PP,H}) & \textsc{Dep-H} & \textsc{F-Pos}\\
  \hline\hline
  á{}-na-henza mu-gaanga &  & $\ast$! & $\ast$ &  & \\
  \hline
  a{}-na-henza mu-gaanga & $\ast$! &  & $\ast$ &  & \\
  \hline
  {\hanr}a{}-na-henza mu-gaángâ &  &  &  &  & $\ast$\\
  \hline
  \end{tabular}
}
\sn
...
\OTbox{
  ...
  \hline
  \normalsize
  ku-henza mu-gaanga & \textsc{Max-H} & \textsc{AR(H,PP)} & \textsc{AR(PP,H)} & \textsc{Dep-H} & \textsc{F-Pos}\\
  \hline\hline
  ku-henza mu-gaanga &  &  & $\ast$! &  & \\
  \hline
  {\hanr} ku-henza mu-gaángâ &  &  &  & $\ast$ & \\
  \hline
  \end{tabular}
  }
\z
...
\hline
\normalsize
 á{}-na-henza mu-gaanga & \textsc{AR(H,PP)} & \textsc{AR(PP,H)} & \textsc{Faith-H} & \textsc{F-Pos}\\
 \hline\hline
 á{}-na-henza mu-gaanga & $\ast$! & $\ast$ &  & \\
 \hline
 a{}-na-henza mu-gaanga &  & $\ast$! & $\ast$ & \\
...
\begin{tabular}{|r|c:c|c:c|}
\hline
 ku-henza mu-gaanga & \textsc{AR(H,PP)} & \textsc{AR(PP,H)} & \textsc{Faith-H} & \textsc{F-Pos}\\
 \hline\hline
 ku-henza mu-gaanga &  & $\ast$! &  & \\
 \hline
 {\hanr} ku-henza mu-gaángâ &  &  & $\ast$ & \\
...
\largerpage
So far, the analysis does not account for why ideophones, unlike other lexical categories, maintain lexical \isi{tone} contrasts in \ili{Tumbuka}. Recall from the discussion in \sectref{sec:downing:3.2}, above, that it is a defining property of ideophones that they must be realized with a particular prosody. In OT terms, this generalization could be formalized as a \textsc{Faith-ProsodyIdeophone} (\textsc{F-IO}) constraint, which is never outranked. (See \citealt{Shih&Inkelas2015} and \citealt{Smith2011} for discussion and analysis of lexical-category specific phonological effects, including category-specific \textsc{faithfulness}.) Since ideophones always end a Phonological Phrase, an \isi{alignment constraint}, \textsc{AlignIdeo} (\textsc{A-IO}), is necessary to optimize that requirement. The analysis is exemplified with the hypothetical example below where word 2 is an \isi{ideophone}:
...
\hline
á{}-na-henza nyuŋumu & \textsc{F-IO} & \textsc{A-IO} & \textsc{AR(H,PP)} & \textsc{AR(PP,H)} & \textsc{Faith-H} & \textsc{F-Pos}\\
 \hline\hline
á{}-na-henza nyuŋumu {\textbar} &  &  & $\ast$! & $\ast$ &  & \\
\hline
a{}-na-henza nyuŋúumu {\textbar} & $\ast$! &  &  &  &  & $\ast$\\
\hline
 {\hanr}a{}-na-henza nyuŋumu {\textbar} &  &  &  & $\ast$ & $\ast$ & \\
\hline
\end{tabular}
}
\z
...
%\textsc{Faith-Ideo} & \textsc{Align-Ideo} & \textsc{Align(H, PhonPh}) & \textsc{Align}\par
