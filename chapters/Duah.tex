\documentclass[output=paper,modfonts,nonflat,colorlinks, citecolor=brown, hidelinks]{langsci/langscibook} 
\ChapterDOI{10.5281/zenodo.3367168}
...
\begin{document}
\lehead{Ọbádélé B. Kambon, Reginald A. Duah \& Clement K. I. Appah}
...
\glt ‘holding and beating’}  & {\cmark} & {\xmark} & {\xmark} & {\xmark}\\

2. & \parbox[t]{4cm}{\gll tɔ-nko-a-da\\
      fall-nod-\textsc{nmlz}-sleep\\
\glt ‘nodding off to sleep’}      & {\xmark} & {\xmark} & {\cmark} & {\xmark}\\

3. & \parbox[t]{4.5cm}{\gll a-wu-n-nya-de(ɛ)\\
\textsc{nmlz}-die-\textsc{nmlz}-leave-thing\\
\glt ‘inheritance’} & {\cmark} & {\cmark} & {\cmark} & {\cmark}\\
\lspbottomrule
\end{tabular}
...
\largerpage
Here, it is also worth noting that intervening elements in SVCNs in general and ISVCNs in particular are the exception rather than the rule with less than ten identified out of just short of 150 attested cases of FL-ISVC nominalization. Further, for the SVCNs with intervening elements, not all speakers produced forms with intervening elements. In fact, it was oftentimes more likely that speakers of \ili{Asante} and \ili{Akuapem} (dialects of \ili{Akan} spoken in different regions of Ghana) would produce forms without intervening elements than that they would produce variants containing them. This begs the question of the motivation for the intervening elements when they do appear. One explanation could be wholly phonological, where the nasal /-N-/ may actually be phonologically conditioned and semantically null. This pattern was typical of \ili{Fante} speakers interviewed in Phase Two
% (P2) 
study groups, in which they regularly produced forms such as \textit{ngyentom} from \textit{gye...to...mu}, \textit{ntwantodo} from \textit{twa...to...so}, \textit{mbɔntohɔ} from \textit{bɔ...to...hɔ} etc. Supplementing this analysis is the idea that, originally, FL-ISVCs were derived from CSCs and ultimately from separate clauses and/or sentences. This progression is illustrated in \figref{fig:duah:2} below. 

\begin{figure}
\begin{tikzpicture}
\node(event1) {\color{lsLightWine}Event1};
\node(event2) [right =3cm of event1] {\color{lsDarkGreenOne}Event2};


\node(csc1) [below=of event1]{\color{lsLightWine}[V1 (Meaning 1)]};
\node(csc2) [below=of event2] {\color{lsDarkGreenOne}[V2 (Meaning 2)]};

\node(plivsc1) [below=of csc1]{\color{lsLightWine}[V1 (Meaning 1)};
\node(plivsc2) [below=of csc2] {\color{lsDarkGreenOne}V2 (Meaning 2)]};

\node(flivsc1) [below=of plivsc1,xshift=10mm]{\color{lsDarkBlue}[V1\vphantom{(}};
\node(flivsc2) [below=of plivsc2,xshift=-10mm] {\color{lsDarkBlue}V2 (Meaning 3)]};

\node(bottom) [below=of flivsc1,xshift=15mm]{\color{lsDarkBlue}[V1+V2 (Meaning 3)]};

\node[align=right,left=of event1, minimum width=4cm] (ev) {Events in perceived world:\vphantom{(}};
\node[align=right,below=of ev, minimum width=4cm] (csc) {CSC:\vphantom{(}};
\node[align=right,below=of csc, minimum width=4cm] (plisvc) {PL-ISVC:\vphantom{(}};
\node[align=right,below=of plisvc,minimum width=4cm] (flisvc) {FL-ISVC:\vphantom{(}};
\node[align=right,below=of flisvc,minimum width=4cm] {PL-ISVC nominalization:\vphantom{(}};

\draw[->,double,thick] (event1) -- (event2);
\draw[->] (event1) -- (csc1);
\draw[->] (csc1) -- (plivsc1);
\draw[->] (plivsc1) -- (flivsc1);
\draw[shorten >=1.5mm,->] (flivsc1) -- (bottom.north);

\draw[transform canvas={yshift=2mm}] (csc1.south east) -- (csc2.south west);
\draw[transform canvas={yshift=2mm}] (plivsc1.south east) -- (plivsc2.south west);
\draw[transform canvas={yshift=2mm}] (csc1.south east) -- (csc2.south west);
\draw[transform canvas={yshift=2mm},color=lsDarkBlue] (flivsc1.south east) -- (flivsc2.south west);

\draw[->] (event2) -- (csc2);
\draw[->] (csc2) -- (plivsc2);
\draw[->] (plivsc2) -- (flivsc2);
\draw[shorten >=1mm,->] (flivsc2) -- (bottom.north);
