...
The most straightforward melody of \ili{Kikamba} causes a single H \isi{tone} to surface on the stem’s \isi{final vowel}. This melody is present, for example, in Habitual forms in “Assertive” clauses (i.e. declarative main clauses without object \isi{focus}). In \REF{ex:jones:3} below, we see such a form in nonfinal position, where it is not affected by the presence of phrasal L tones to be discussed in \sectref{sec:jones:4.2}. Following \citet{Roberts-Kohno2014}, we analyze this melody as \{H\textsubscript{FV}\}.
...
\texttoptiebar{HL} permitted only pre-pausally \citep[252]{Roberts-Kohno2000}\\
